package cn.sharpen.jctool.util.algorithm.geo;
...
/**
 ...
 ————————————————
 https://blog.csdn.net/xxxx000/article/details/92771460
 */
...
public final class GeoHash implements Comparable<GeoHash>, Serializable {
    ...
    public static final int DEF_PRECISION= 60;
    /**
     * 谷歌地图坐标转geohash
     * @param coordinate 例：30.5964594583835, 114.27950524640751
     * @return
     */
    public static String geoHahByGoogleMap(String coordinate){
        if(coordinate==null || !coordinate.contains(COMMA)) {
            return null;
        }
        String[] strs = coordinate.split(COMMA);
        GeoHash g = new GeoHash(Double.parseDouble(strs[0].trim()), Double.parseDouble(strs[1].trim()), DEF_PRECISION);
        return g.toBase32();
    }
    /**
     * 百度地图坐标转geohash
     * 获取坐标：  https://api.map.baidu.com/lbsapi/getpoint/index.html
     * @param coordinate 例：112.194755,32.114433
     * @return
     */
    public static String geoHahByBaiduMap(String coordinate){
        if(coordinate==null || !coordinate.contains(COMMA)) {
            return null;
        }
        String[] strs = coordinate.split(coordinate);
        GeoHash g = new GeoHash(Double.parseDouble(strs[1]), Double.parseDouble(strs[0]), DEF_PRECISION);
        return g.toBase32();
    }
